...
module.exports = [
    ...
    //                                                                         };
    //                                                                         // insert  analytics profile to db
    //                                                                         const insertAnalytics: any = mongo.collection('analytics').insertOne(analytics)
    ...
    //         const mongo = Util.getDb(request)
    //         try {
    //             const resAnalytics = await mongo.collection('analytics').findOne({ _id: ObjectId(request.params._id) })
    //             reply({
    //                 statusCode: 200,
    //                 message: "OK",
    //                 data: resAnalytics
    ...
    //                 const updateAnalytics: any = await mongo.collection('analytics').updateOne({ _id: ObjectId(payload.id) }, { $set: payload })
    //                 if (updateAnalytics.acknowledged) {
    //                     const updateAssignAnalytics: any = await mongo.collection('assignAnalytics').updateOne({ _refAnalyticsId: payload.id }, { $set: payload })
    //                     if (updateAssignAnalytics.acknowledged) {
    //                         reply({
    //                             statusCode: 200,
    //                             message: "OK",
    //                             data: "Update Succeed"
    ...
    //             let payload = request.payload;
    //             const mongo = Util.getDb(request)
    //             const resAnalytics = await mongo.collection('analytics').findOne({ _id: ObjectId(payload._id) })
    //             if (resAnalytics) {
    ...
    //                             badRequest("Error " + error)
    //                         } else if (stdout) {
    //                             const removeAnalytics: any = mongo.collection('analytics').deleteOne({ _id: ObjectId(payload._id) })
    //                             if (removeAnalytics.acknowledged) {
    //                                 reply({
    //                                     statusCode: 200,
    //                                     message: "OK",
    //                                 })
    ...
    //         const mongo = Util.getDb(request)
    //         try {
    //             const resAnalytics = await mongo.collection('analytics').findOne({ _id: ObjectId(request.params.id) })
    //             if (resAnalytics) {
    //                 // if not image
    //                 var contentType
